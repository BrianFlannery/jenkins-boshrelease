...
pid_guard() {
  ...
  if [ -f "$pidfile" ]; then
    ...
    #[[ $output_label = $name ]] is not the best solution but ensures we are checking the pid of the right
    if [ -n "$pid" ] && [ -e /proc/$pid ] ; then
      echo "$name is already running with PID $pid, please stop it first"
      exit 1
    fi
